#' Find the Next Lambda Value for Regularization Path
#'
...
#find_next_lambda function v1
find_next_lambda <- function(y, current_lambda, B, S) {
  ...
  if (is.null(B) || length(B) == 0) {
    ...
    D_minus_B <- D
    M_inv <- solve(D %*% t(D))  # Inverse of D_{-B} * D_{-B}^T
    D_B <- NULL
  } else {
    # Extract submatrices based on the current boundary set B
    D_minus_B <- D[-sort(B), , drop = FALSE]
    M_inv <- solve(D_minus_B %*% t(D_minus_B))  # Inverse for reduced matrix
